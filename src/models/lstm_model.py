...
# ========================================================
"""This module is written for write LSTM classifier."""
# ========================================================
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, idx2tag: dict, tag2idx: dict, pad_token: str, num_vocab, config):
        super().__init__()
        self.config = config
        ...
        self.network = torch.nn.ModuleDict({
            "embedding_layer": torch.nn.Embedding(num_embeddings=num_vocab,
                                                  embedding_dim=config.embedding_dim,
                                                  padding_idx=self.tag2idx[self.pad_token]),
            ...
            "lstm_layer_1": torch.nn.LSTM(input_size=config.embedding_dim,
    ...
    def forward(self, batch):
        ...
        :return:
        """
        embedding_output = self.network["embedding_layer"](batch["input_ids"])
        # embedding_output.size() = [batch_size, sen_len, embedding_dim]
        # ------------------------------ LSTM Block ------------------------------#
        lstm_output, (_, _) = self.network["lstm_layer_1"](embedding_output)
        lstm_output = self.network["activation_fn"](lstm_output)
        lstm_output = self.network["dropout"](lstm_output)
        # lstm_output.size() = [batch_size, sen_len, bidirectional*lstm_units]
        ...
        out = self.network["output_layer"](lstm_output)
        return out
    ...
    def extract_tags(self) -> list:
        ...
        tags = tags - {self.pad_token, "O", "[SEP]", "[CLS]"}
        return list(tags)
    ...
    def training_step(self, batch: dict, _):
        ...
        metric_names, metric_value = self.classification_f1_report(true_targets,
                                                                   pred_targets,
                                                                   data_name="train")
    ...
    def classification_f1_report(self, true_targets_value: List[list],
                                 pred_targets_value: List[list], data_name: str) -> [list, list]:
        """
