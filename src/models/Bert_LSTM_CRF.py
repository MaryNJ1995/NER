...
from models.attention import DotProductAttention, ScaledDotProductAttention
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, idx2tag: dict, tag2idx: dict, pad_token: str, config):
        ...
        self.crf_layer = ConditionalRandomField(num_tags=len(self.tag2idx),
    ...
    def forward(self, batch, labels=None):
        ...
        token_scores = self.network["output_layer"](lstm_output)

        if labels is not None:
            loss = -self.crf_layer(token_scores, labels, batch["attention_mask"]) / float(token_scores.size(0))
            prediction = self.crf_layer.viterbi_tags(token_scores, batch["attention_mask"])
    ...
    def _convert_pred_indexes_to_entities(self, true_indexes: torch.Tensor,
        ...
        pred_indexes = [item[0] for item in pred_indexes]
