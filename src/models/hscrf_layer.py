...
class HSCRF(nn.Module):
    ...
    def __init__(self, tag_to_ix, word_rep_dim=300, SCRF_feature_dim=100, index_embeds_dim=10, ALLOWED_SPANLEN=6,
                 start_id=4, stop_id=5, noBIES=False, no_index=False, no_sub=False, grconv=False):
    ...
    def get_logloss_denominator(self, scores, mask):
        ...
        logalpha = Variable(
            torch.FloatTensor(self.batch_size, self.sent_len + 1, self.tagset_size).fill_(-10000.)).cuda()
        ...
        for i in range(1, self.sent_len + 1):
            tmp = scores[:, istarts[i]:i, i - 1] + \
                  logalpha[:, istarts[i]:i].unsqueeze(3).expand(self.batch_size, i - istarts[i], self.tagset_size,
                                                                self.tagset_size)
            tmp = tmp.transpose(1, 3).contiguous().view(self.batch_size, self.tagset_size,
                                                        (i - istarts[i]) * self.tagset_size)
    ...
    def decode(self, factexprscalars, mask):
        ...
        for j in range(1, sentlen + 1):
            ...
            f = factexprscalars[:, istart:j, j - 1].permute(0, 3, 1, 2).contiguous().view(batch_size, self.tagset_size,
                                                                                          -1) + \
                logalpha[:, istart:j].contiguous().view(batch_size, 1, -1).expand(batch_size, self.tagset_size,
                                                                                  (j - istart) * self.tagset_size)
    ...
    def get_logloss_numerator(self, goldfactors, scores, mask):
        ...
        goldfactors = goldfactors[:, :, 0] * sent_len * tagset_size * tagset_size + goldfactors[:, :,
                                                                                    1] * tagset_size * tagset_size + goldfactors[
                                                                                                                     :,
                                                                                                                     :,
                                                                                                                     2] * tagset_size + goldfactors[
                                                                                                                                        :,
                                                                                                                                        :,
                                                                                                                                        3]
    ...
    def grConv_scores(self, feats):
        ...
        for span_len in range(1, min(self.ALLOWED_SPANLEN, self.sent_len)):
            ...
            w = torch.exp(self.Gl(ht_1_l) + self.Gr(ht_1_r)).view(self.batch_size, self.sent_len - span_len, 3,
                                                                  self.SCRF_feature_dim).permute(2, 0, 1, 3)
    ...
    def HSCRF_scores(self, feats):
        ...
        scores = Variable(
            torch.zeros(self.batch_size, self.sent_len, self.sent_len, self.tagset_size, self.tagset_size)).cuda()
        diag0 = torch.LongTensor(range(self.sent_len)).cuda()
        # m10000 for STOP
        m10000 = Variable(
            torch.FloatTensor([-10000.]).expand(self.batch_size, self.sent_len, self.tagset_size, 1)).cuda()
        # m30000 for STOP, START, O
        m30000 = Variable(
            torch.FloatTensor([-10000.]).expand(self.batch_size, self.sent_len, self.tagset_size, 3)).cuda()
        for span_len in range(min(self.ALLOWED_SPANLEN, self.sent_len)):
            ...
            if span_len == 0:
                tmp = torch.cat((self.transition[:, :validtag_size].unsqueeze(0).unsqueeze(0) + emb_x[:, 0, :,
                                                                                                :validtag_size].unsqueeze(
                    2),
                                 m10000,
                ...
                tmp = torch.cat((self.transition[:, :validtag_size].unsqueeze(0).unsqueeze(0).expand(self.batch_size,
                                                                                                     self.sent_len - 1,
                                                                                                     self.tagset_size,
                                                                                                     validtag_size) + \
                ...
                tmp0 = self.transition[:, :validtag_size].unsqueeze(0).unsqueeze(0).expand(self.batch_size,
                                                                                           self.sent_len - span_len,
                                                                                           self.tagset_size,
                                                                                           validtag_size) + \
                       (emb_x[:, 0, :, validtag_size:2 * validtag_size] +
    ...
    def concat_features(self, emb_z, span_len):
        ...
        new_emb_z2 = (new_emb_z1[:, :, 0] - new_emb_z1[:, :, span_len]).unsqueeze(2).expand(batch_size,
                                                                                            sent_len - span_len,
                                                                                            span_len + 1, hidden_dim)
        index = Variable(torch.LongTensor(range(span_len + 1))).cuda()
        index = self.index_embeds(index).unsqueeze(0).unsqueeze(0).expand(batch_size, sent_len - span_len, span_len + 1,
                                                                          self.index_embeds_dim)
