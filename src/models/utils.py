...
from models.data_packer import CRFDataset_WC
...
def load_embedding(emb_file, delimiter, feature_map, full_feature_set, unk, emb_len, shrink_to_train=False,
...
def construct_bucket_mean_vb_wc(word_features, input_label, label_dict, SCRF_label_dict, char_dict, word_dict,
                                SCRF_stop_tag, ALLOW_SPANLEN=6, train_set=False):
    """
    ...
    dataset = construct_bucket_vb_wc(new_word_features, forw_features, fea_len, new_labels, new_SCRFlabels,
                                     char_features,
...
def construct_bucket_vb_wc(word_features, forw_features, fea_len, input_labels, SCRFlabels, char_features, thresholds,
                           pad_word_feature, pad_char_feature, pad_label, label_size, SCRF_stop_tag):
    """
    ...
    for f_f, f_l, w_f, i_l, s_l, c_f in zip(forw_features, fea_len, word_features, input_labels, SCRFlabels,
                                            char_features):
        cur_len = len(f_l)
        idx = 0
        cur_len_1 = cur_len + 1
        ...
        padded_feature = f_f + [pad_char_feature] * (
                    buckets_len[idx] - len(f_f))  # pad feature with <'\n'>, at least one

        padded_feature_len = f_l + [1] * (thresholds[idx] - len(f_l))  # pad feature length with <'\n'>, at least one

        padded_feature_len_cum = list(
            accumulate(padded_feature_len))  # start from 0, but the first is ' ', so the position need not to be -1
        buckets[idx][0].append(padded_feature)  # char
        buckets[idx][1].append(padded_feature_len_cum)
        buckets[idx][2].append(padded_feature[::-1])
        buckets[idx][3].append(
            [buckets_len[idx] - 1] + [buckets_len[idx] - 1 - tup for tup in padded_feature_len_cum[:-1]])
        buckets[idx][4].append(w_f + [pad_word_feature] * (thresholds[idx] - cur_len))  # word
        buckets[idx][5].append([i_l[ind] * label_size + i_l[ind + 1] for ind in range(0, cur_len)] + [
        ...
        buckets[idx][8].append(
            s_l + [[s_l[-1][1] + 1, s_l[-1][1] + 1, s_l[-1][-1], SCRF_stop_tag]] + [[0, 0, 0, 0] for _ in range(
                thresholds[idx] - cur_scrf_len_1)])
        buckets[idx][9].append([1] * cur_scrf_len_1 + [0] * (thresholds[idx] - cur_scrf_len_1))
        buckets[idx][10].append(
...
def scrf_to_crf(decoded_scrf, l_map):
    ...
    for i_l in crf_labels:
        cur_len_1 = len(i_l)
        cur_len = cur_len_1 - 1
        i_l_pad = [i_l[ind] * label_size + i_l[ind + 1] for ind in range(0, cur_len)] + [
...
def decode_with_crf(crf, word_reps, mask_v, l_map):
    ...
    tg_energy = torch.gather(scores.view(seq_len, bat_size, -1), 2, bi_crf).view(seq_len,
...
def rescored_with_crf(decoded_scrf, l_map, scores):
    ...
    tg_energy = torch.gather(scores.view(seq_len, bat_size, -1), 2, scrfdata).view(seq_len,
                                                                                   bat_size)  # seq_len * bat_size
